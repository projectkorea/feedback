...
export class Form {
  ...
  render(): HTMLFormElement {
    ...
    this.form.querySelectorAll<HTMLButtonElement>('.rating-btn').forEach((btn) => {
      btn.addEventListener('click', () => {
        this.form!.querySelectorAll<HTMLButtonElement>('.rating-btn').forEach((b) => {
          b.style.borderColor = '#e5e7eb';
          b.style.background = 'white';
        });
        btn.style.borderColor = '#4F46E5';
        btn.style.background = '#EEF2FF';
        this.selectedRating = parseInt(btn.dataset.rating || '0');
      });
    });

    this.form.addEventListener('submit', async (e) => {
      e.preventDefault();
      const formData = new globalThis.FormData(this.form!);
      const data: FormData = {
