...
interface SQLiteSettings {
  ...
  notifications_slack: number;
  notifications_discord: number;
  notifications_telegram: number;
...
export class SQLiteAdapter extends DatabaseAdapter {
  ...
  async createProject(data: ProjectCreateData): Promise<Project> {
    ...
    const project = this.db!.prepare('SELECT * FROM projects WHERE id = ?').get(data.id) as SQLiteProject;
  ...
  async getProjectByKey(projectId: string, publicKey: string): Promise<Project | null> {
    ...
    const project = stmt.get(projectId, publicKey) as SQLiteProject | undefined;
  ...
  async getAllProjects(): Promise<Project[]> {
    ...
    const projects = stmt.all() as SQLiteProject[];
  ...
  async getFeedbackList(projectId: string, filters: FeedbackListFilters = {}): Promise<FeedbackListResult> {
    ...
    const items = stmt.all(...params, Number(limit), offset) as FeedbackListResult['items'];
  ...
  async getFeedbackStats(projectId: string): Promise<FeedbackStats[]> {
    ...
    return stmt.all(projectId) as FeedbackStats[];
  }
  ...
  async getSettings(projectId: string): Promise<Settings | null> {
    ...
    const settings = stmt.get(projectId) as SQLiteSettings | undefined;
    ...
    return {
      ...
      customization: {
        ...
        position: settings.custom_position as Settings['customization']['position'],
  ...
  async updateSettings(projectId: string, data: SettingsUpdateData): Promise<Settings | null> {
    ...
    stmt.run(
      projectId,
      data.notifications?.enabled !== false ? 1 : 0,
