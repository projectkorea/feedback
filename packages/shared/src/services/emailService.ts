...
import type { EmailConfig, Feedback } from '../types.js';
...
interface FeedbackNotification extends Partial<Feedback> {
  id?: string;
  category: string;
  rating?: number;
  message?: string;
...
export class EmailService {
  ...
  async sendFeedbackNotification(feedback: FeedbackNotification): Promise<void> {
    if (!this.transporter) {
    ...
    try {
      ...
      const info = await this.transporter.sendMail({
        ...
        html,
        text: this.generatePlainText(feedback),
      });
  ...
  private generatePlainText(feedback: FeedbackNotification): string {
    const categoryEmoji = CATEGORY_EMOJIS[feedback.category] || '📝';
    const ratingStars = feedback.rating ? '⭐'.repeat(feedback.rating) : '평점 없음';
